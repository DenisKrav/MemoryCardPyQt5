...
class QuestionListModel(QAbstractListModel):
    ...
    def insertRows(self, parent=QModelIndex()):
        ...
        self.beginInsertRows(parent, position,
                             position)  # вставка данных должна быть после этого указания и перед endInsertRows()
    ...
    def removeRows(self, position, parent=QModelIndex()):
        ...
        self.beginRemoveRows(parent, position,
                             position)  # сообщаем, что мы собираемся удалять строку - от position до position
...
def random_AnswerCheck(list_model, w_question, widgets_list, w_showed_answer, w_result):
    ...
    frm_card = AnswerCheck(frm, w_question, widgets_list[0], widgets_list[1], widgets_list[2], widgets_list[3],
                           w_showed_answer, w_result)
    return frm_card